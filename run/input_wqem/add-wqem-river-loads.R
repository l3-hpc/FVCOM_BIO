...
tpfile <- "/Users/lllowe/FVCOM_WQEM/2013_leem_fine_river_data.nc"
...
for(i in 1:length(wqem_vars)){
  value <- ncvar_def( name=wqem_vars[i], units=wqem_units[i], dim=list(dimrivers, dimtime), missval=mv2, 
                      longname=paste("river total",wqem_vars[i],"load"))
...
for(itime in 1:nt){
  for(iriv in 1:nr){
    values[iriv,itime] <- wqem_init_values[i]
      #as.numeric(df[itime,iriv])
  }
}
ncvar_put(ncdata, varid=wqem_vars[i], vals=values, start=c(1,1), count=c(-1,-1), verbose=FALSE )
}
